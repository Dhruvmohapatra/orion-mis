    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6 max-w-4xl">
      ...
      <div className="glass-card p-6">
        <h3 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
          <ChatBubbleLeftRightIcon className="w-5 h-5 text-indigo-600" /> Student Queries Inbox
        </h3>
          ...
          <div className="space-y-4">
            {messages.map((m) => (
              <div key={m.message_id} className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-2">
                ...
                <h4 className="font-bold text-slate-900 text-base">{m.subject}</h4>
                <p className="text-xs text-slate-600 bg-white p-3 rounded-lg leading-relaxed">{m.content}</p>
              </div>
            ))}
          </div>
        )}
